...
JsSketch.prototype.install = function (then) {
  this.storage.install(then);
};

JsSketch.prototype.Error = function (information, isApplication) {
  var e = new Error(information);

  Object.defineProperty(e, 'isApplication', {
    enumerable: true,
    value: !!isApplication
  });
...
JsSketch.prototype.loadRevision = function (revisionId, sketchId, callback) {
  ...
  this.storage.getRevision(revisionId, sketchId, function (err, revisionProperties, cssAssets, jsAssets) {
    if (err) {
...
JsSketch.prototype.saveAjax = function (ajax, callback) {
  ...
  if (ajax.validate() === true) {
    this.storage.saveAjax(ajax.properties, function (err, id) {
      if (err) {
        callback(that._mapToApplicationError(err));
      } else {
        ajax.set('id', id);
...
/**
 * Retrieves the AJAX models associated with a particular revision.
 *
 * @param sketch Sketch ID (string)
 * @param revision Revision ID (number)
 * @param callback(err, array) array will be of AJAX models. Empty if no AJAX
 *        requests are associated with the revision.
 */
...
/**
 * Associates the AJAX models with a particular revision. The AJAX requests will
 * already have been saved.
 *
 * @param ajaxRequests Array of AJAX models
 * @param sketch Sketch ID (string)
 * @param revision Revision ID (number)
 * @param callback(err)
 */
JsSketch.prototype.saveAjaxForRevision = function (ajaxRequests, sketch, revision, callback) {
  ...
  if (ajaxRequests.length) {
    this.storage.saveAjaxForRevision(sketch, revision, ajaxRequests.map(function (ajaxModel) {
      return ajaxModel.properties;
    }), function (err) {
      callback(that._mapToApplicationError(err));
    });
  } else {
