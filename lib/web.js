...
module.exports.init = function (config) {
  ...
  (function () {
    var folder = __dirname  + '/../' + (config.production ? 'build' : 'public') + '/';

    // Use express.static for all our static endpoints.
    ["css","js","codemirror","img"].forEach(function (dir) {
      base.use('/' + dir, express.static(folder + dir));
    });
   ...
   */
  var render = (function () {
    var requirejs = utils.readJsonFileSync(__dirname + '/../config/requirejs');
    var bundles = {};

    Object.keys(config.bundles).forEach(function (name) {
      var curr = this[name];

      bundles[name] = {
        css: (config.production ? ['/css/all.' + name + '.css'] : curr.css) || [],
        js: (curr.js ? ['/js/' + name + '.js'] : [])
      };
    }, config.bundles);
  ...
  function preview(sketch, revision, res) {
    render('preview.hbs', {
      ...
      revision: revision,
      dm: client.dm,
      doctype: client.doctypes[revision.get('doctype')]
    }, res);
  }
